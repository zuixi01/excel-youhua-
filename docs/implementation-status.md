...
- 当前完整本地生产 Renderer 回归：`147 passed, 7 skipped`（共收集 154 项）；条件跳过的性能、真实基础设施和 LibreOffice 已由专用远程作业承接。
- 当前远端提交 `0534ff2f41807fc481a0ff59d80c162ae5c2ee5d` 的 [主 CI](https://github.com/zuixi01/excel-youhua-/actions/runs/33252789525) 全部绿色：Python 3.12/3.14、Web、Windows Renderer、LibreOffice、真实 PostgreSQL/Redis/RQ/MinIO 和依赖安全共七项均通过；JUnit、Coverage、LibreOffice、Renderer、基础设施及依赖治理制品均已保存。
- 同一提交的 [性能基线 v4 最终复核运行](https://github.com/zuixi01/excel-youhua-/actions/runs/33252958226) 八项全部绿色：五个工作簿负载场景、50,000 条分页标准源、500,000 条直接比较，以及 500,000 条受管 HTTP 服务全链路均通过相对回归门禁并保存指标及 JUnit 制品。
...
- 仓库记录的 v4 参考性能包括：10k×50 为 14.416 秒/50.48 MiB；100k×100 为 41.054 秒/254.23 MiB；100k×200、50% 差异为 53.641 秒/287.38 MiB；5 工作表 50k×50 为 11.045 秒/18.2 MiB；50,000 条分页标准源为 0.161 秒。500,000 条直接比较为 78.03 秒/268.13 MiB；受管 HTTP 服务全链路为 155.871 秒/303.38 MiB，并生成约 549 MiB 的 JSON、JSONL、HTML 工件。详见 `docs/performance-baselines.json`。
