...
工作表别名解析同样不允许静默任选：一个规则若同时匹配规范表名与一个或多个别名工作表，任务生成 `AMBIGUOUS_SHEET` 并进入人工审核，不执行记录匹配或渲染；受管多表标准响应若同时用 sheet id 与 sheet name 指向同一规则，则在快照前以 `STANDARD_DATA_INVALID` 拒绝。开发降级渲染器与生产 manifest 均使用已经确认的物理工作表名。显式数据起始行必须晚于表头；自动定位后发生重叠会进入人工审核。工作表最大数据行数从实际首数据行计算，既不把前导说明区误计为数据，也不放过超限记录。
...
- 当前完整本地生产 Renderer 回归：`243 passed, 8 skipped`（共收集 251 项）；条件跳过的性能、真实基础设施和 LibreOffice 已由专用远程作业承接。
