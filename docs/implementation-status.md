...
核心精度入口现统一拒绝会产生静默选择或值丢失的标准数据：规范字段名、展示名和别名按 NFKC/空白规则映射，同一字段的冲突表示、JSON 重复对象键、CSV 重复字段名、HTTP 非严格 JSON 均在快照前失败。不同物理表头即使文字不同，只要映射到同一规范字段也按重复表头处理，不会由后列覆盖前列。标准端和 Excel 端的长度、正则、枚举、唯一性均基于相同的规范化值语义；规则发布会拒绝负/非有限容差、矛盾范围、类型不兼容选项、歧义枚举和违反自身校验的静态默认值，也会拒绝重复工作表别名、空白字段标题、非法规范字段标识、无效集合分隔符，以及经标准化或正则处理后永远无法命中的值别名/枚举。字符串长度/正则、日期时间精度和日期解析格式都按字段类型做发布期校验，不会再对数值的字符串表示误判或接受运行时无效选项。内置跨字段校验器使用精确参数契约，拒绝缺失/拼错参数、无效条件值、类型不兼容的日期字段和重复规则 ID，避免规则被静默跳过。主键字段必须使用确定性的精确比较、禁止模糊类型和公式模式；Excel 公式主键会被排除出匹配索引、产生明确差异并转人工审核。空主键的 `report_invalid`、`skip_row`、`use_row_number` 在标准校验和比较引擎中使用同一语义：跳过行不再被其他字段误阻断，行号回退仍严格校验非主键字段。显式 `row_number` 主键的伪字段会在上传标准规范化时保留，且只有标准行号等于实际下一物理行时才允许自动追加，避免把记录写入错误行。类型化修复使用与比较相同的规范值写入：布尔、枚举、百分比 Decimal、日期时间时区、集合和 JSON 均保持 Excel 类型/稳定表示，而报告继续保留标准原始值用于审计。Decimal 非有限值及大精度量化有确定性边界；标准记录省略可选字段不会触发自动清空，只有显式空值才进入已授权覆盖路径。
...
- 当前完整本地生产 Renderer 回归：`244 passed, 8 skipped`（共收集 252 项）；条件跳过的性能、真实基础设施和 LibreOffice 已由专用远程作业承接。
