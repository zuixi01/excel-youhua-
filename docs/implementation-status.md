...
- 当前完整本地生产 Renderer 回归：`247 passed, 8 skipped`（共收集 255 项）；条件跳过的性能、真实基础设施和 LibreOffice 已由专用远程作业承接。
